...
def _preprocess_data(data):
    ...
    feature_vector_df['Seville_pressure'] = feature_vector_df['Seville_pressure'].astype(str).str.extract('(\d+)', expand=False).astype(int)
    ### reformatting the data to the correct type
    feature_vector_df['Valencia_wind_deg'] = feature_vector_df['Valencia_wind_deg'].astype(str).str.extract('(\d+)', expand=False).astype(int)
    ### Valencia_pressure has 2068 out of 8763 total values missing

    feature_vector_df['Valencia_pressure'].fillna(feature_vector_df['Valencia_pressure'].median(), inplace=True) # mean = 1012.0514065222798  , mode = 1018 , median = 1015

    # for col in feature_vector_df.columns:
    #     if col == 'time':
    #         continue
    #     value = feature_vector_df[col].median()
    #     feature_vector_df[col].fillna(value, inplace=True)
